...
const orderItemSchema = t.Object({
  ...
  supplier_price_cents: t.Optional(t.Number())
});

const validStatuses = ["draft", "pending", "preparing", "shipped", "completed", "cancelled"] as const;

export const ordersRoutes = new Elysia({ prefix: "/orders" })
  .use(dbPlugin)

  // GET /orders/reconcile - must be before /:id to avoid route conflict
  .get(
    "/reconcile",
    async ({ sql, query }) => {
      ...
      if (query.customer_id) {
        conditions.push(`so.customer_id = $${conditions.length + 1}`);
  ...
  // POST /orders - create order (supports auto-create product/customer)
  .post(
    "/",
    async ({ sql, body, set }) => {
      ...
      return await sql.begin(async (_tx: any) => {
        ...
        if (!customerId) {
          set.status = 400;
          return { message: "customer_id or customer_name is required" };
        }

        const orderNo = await generateOrderNumber(tx);

        const orderRows = await tx.unsafe(
          `insert into sales_orders (customer_id, order_no, source, status, note, created_by)
           values ($1, $2, $3, $4, $5, $6) returning *`,
          [customerId, orderNo, body.source ?? "manual", body.status ?? "pending", body.note ?? null, body.created_by ?? null]
        );
        ...
        for (const item of body.items ?? []) {
          ...
          if (!productId && item.product_name) {
            ...
            if (existing.length > 0 && existing[0]) {
              ...
              const created = await tx.unsafe(
                ...
                [item.product_name, item.unit_price_cents]
              );
          ...
          const itemRows = await tx.unsafe(
            `insert into sales_order_items
              (sales_order_id, product_id, product_name, quantity, unit_price_cents, line_total_cents, supplier_id, supplier_price_cents)
             values ($1, $2, $3, $4, $5, $6, $7, $8) returning *`,
            [order!.id, productId, item.product_name, item.quantity, item.unit_price_cents, lineTotalCents, item.supplier_id ?? null, item.supplier_price_cents ?? 0]
          );
  ...
  .get(
    ...
    async ({ sql, params, set }) => {
      ...
      const items = await sql`
        select soi.*, p.barcode, p.spec, p.oe_code, p.unit
